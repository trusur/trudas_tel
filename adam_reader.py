                    ...
                    if(config[0] == 1):
                        ...
                        if(ain[8] > 0):
                            ...
                            if(rca_reader_value != 0):
                                ...
                                sql_update_log = (
                                    "UPDATE sensor_value_rca_logs SET data = '" + str(correction) + "', voltage = '" + str(eval(ain[6])) + "' WHERE instrument_param_id = '" + str(ain[3]) + "'")
                                mycursor.execute(sql_update_log)
                                mydb.commit()
                                ...
                                sql_insert_log = (
                                    "INSERT INTO sensor_value_rca_logs (instrument_param_id, data, voltage, xtimestamp) VALUES ('" + str(ain[3]) + "','" + str(correction) + "','" + str(eval(ain[6])) + "','" + timestamp + "')")
                                mycursor.execute(sql_insert_log)
                                mydb.commit()
