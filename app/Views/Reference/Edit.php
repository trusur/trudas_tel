...
<div class="container">
    ...
    <div class="row justify-content-center">
        <div class="col-6">
            <form action="<?= base_url('reference/update') ?>" method="POST">
                <div class="row">
                    <div class="col-12 mb-3">
                        ...
                        <select name="instrument_param_id" class="form-select <?= $validation->hasError('instrument_param_id') ? 'is-invalid' : '' ?>">
                            <?php foreach ($sensors as $sensor) : ?>
                                <option value="<?= $sensor->instrument_param_id ?>" <?= empty(old('instrument_param_id')) ? ($sensor->instrument_param_id == $reference->instrument_param_id ? 'selected' : '') : (old('instrument_param_id') == $sensor->instrument_param_id ? 'selected' : '') ?>><?= $sensor->sensor_code ?></option>
                            <?php endforeach ?>
                        </select>
                    ...
                    <div class="col-6 mb-3">
                        ...
                        <input type="number" name="range_start" class="form-control <?= $validation->hasError('range_start') ? 'is-invalid' : '' ?>" placeholder="ex : 1" value="<?= !empty(old('range_start')) ? old('range_start') : $reference->range_start ?>">
                        <small class="text-danger">
                    ...
                    <div class="col-6 mb-3">
                        ...
                        <input type="number" name="range_end" class="form-control <?= $validation->hasError('range_end') ? 'is-invalid' : '' ?>" placeholder="ex : 30" value="<?= !empty(old('range_end')) ? old('range_end') : $reference->range_end ?>">
                        <small class="text-danger">
